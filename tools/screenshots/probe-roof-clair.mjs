...
await page.screenshot({ path: resolve(OUT, 'roof-clair-3d.png') })
console.log('✔ roof-clair-3d.png')
const plan = sheet.getByRole('button', { name: 'Plan', exact: true })
if (await plan.count()) {
  await plan.first().click()
  await page.waitForTimeout(1500)
  await page.screenshot({ path: resolve(OUT, 'roof-clair-plan.png') })
  console.log('✔ roof-clair-plan.png')
}
const report = sheet.getByRole('button', { name: 'Rapport', exact: true })
if (await report.count()) {
  await report.first().click()
  await page.waitForTimeout(2500)
  await page.screenshot({ path: resolve(OUT, 'roof-clair-rapport.png') })
  console.log('✔ roof-clair-rapport.png')
  await page.getByRole('button', { name: 'Fermer le rapport' }).click({ force: true })
}
await browser.close()
console.log('✔ probe-roof-clair')
