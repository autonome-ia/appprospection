...
- [x] **Maquette 3D du toit dans la fiche (fin juillet 2026)** : bouton « Voir le toit en 3D » (fiche point + fiche avant prospection) → toit du client manipulable au doigt, three.js en chunk séparé chargé au tap. En parallèle : **carte 3D (extrusion) retirée**. **v2 après premiers tests briac (rotation morte, trous, « épées »)** : pans **jointifs** reconstruits par partition de l'emprise (`lidar-recon.ts`, testé — mêmes polygones propres sur l'ortho), murs à hauteur réelle (BD TOPO) + pignons, ombre portée, entrée animée + autorotation, pastilles m² sur les pans, boussole, plein écran démo. `LIDAR_VERSION` 7 (re-mesure paresseuse). Avis du chef des ventes à recueillir sur l'effet en rendez-vous.
