...
- [ ] ⬜ **Gate G0 en conditions réelles** : le chef des ventes tape ses chantiers passés dans l'app et compare le badge aux surfaces facturées. Si recalibrage → bump `LIDAR_VERSION` (domain/house.ts).
- [ ] ⬜ Phase 3 — pans dessinés sur l'ortho dans la fiche (option, après G0).
