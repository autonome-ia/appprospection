...
- [x] **Précision toiture (retour briac : « Autre » trop vague)** : la nomenclature fiscale n'a que 5 catégories (limite de la source, pas un réglage) — 3 améliorations : matériau **secondaire** affiché (« Tuiles + zinc/alu »), « Autre » sur une maison ≤ 1997 devient un badge d'alerte **« Autre — fibro ? »** (signal amiante), et champ **« Toiture constatée »** sur la fiche (liste métier, **migration `db/0007_mat_toit_confirme.sql` à exécuter**) qui remplace la donnée fiscale — badge bleu « confirmé ». Liste des matériaux à affiner avec le chef des ventes.
