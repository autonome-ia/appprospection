...
- [x] **Maquette 3D du toit dans la fiche (fin juillet 2026)** : bouton « Voir le toit en 3D » (fiche point + fiche avant prospection) → toit du client manipulable au doigt, pans colorés (palette partagée avec l'ortho) + légende m²/pente. `LIDAR_VERSION` 6 (altitudes par sommet de contour, re-mesure paresseuse), three.js en chunk séparé chargé au tap (~139 Ko gz). En parallèle : **carte 3D (extrusion) retirée**. Avis du chef des ventes à recueillir sur l'effet en rendez-vous.
