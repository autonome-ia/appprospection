...
27. 🟡 **Contacts saisis à la main et statistiques** : depuis le 27/07, un contact peut être créé manuellement (bouton « + » de la vue Contacts) — il pose un point et est donc **compté comme une porte frappée** dans les stats. OK si c'est la ressaisie d'une vraie tournée ; à revoir si vous importez d'anciens contacts en masse (on ajouterait alors un marquage pour les exclure du comptage). Même question pour les **« Ancien client » ressaisis en masse** (29/07) : chaque saisie compte une porte ce jour-là (jamais une vente — le tunnel reste juste).
28. 🟡 **Les « Anciens clients » sont-ils un actif d'AGENCE ou du commercial ?** Aujourd'hui la vue Contacts montre à chacun SES contacts (le manager voit tout) — un ancien client marqué par X n'apparaît donc pas dans la liste de Y (il le voit sur la carte, qui est partagée). Si le carnet des anciens clients doit être commun à l'équipe (références de rue, entretien/R2), on l'ouvrira à tous dans la vue Contacts — à trancher avec le chantier Équipe.
